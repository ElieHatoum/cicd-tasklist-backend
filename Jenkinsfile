pipeline {
    ...
    environment {
        // ----- Docker Hub -----
        IMAGE_NAME = 'eliehatoum/cicd-tasklist-backend'
        IMAGE_TAG  = "${BUILD_NUMBER}"            // image tagged with the Jenkins build number
        IMAGE      = "${IMAGE_NAME}:${IMAGE_TAG}"

        // ----- SonarQube -----
        SONAR_HOST_URL  = 'https://sonarqube.cicd.kits.ext.educentre.fr'
        SONAR_PROJECT_KEY = 'Elie-cicd-tasklist-backend'
    }
    ...
    stages {
        ...
        stage('SonarQube analysis') {
            steps {
                withCredentials([string(credentialsId: 'sonar-tasklist-backend', variable: 'SONAR_TOKEN')]) {
                    sh '''
                        sonar-scanner \
